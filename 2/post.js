...
const separateFragmentShader = `precision highp float;
...
class Post {
  constructor(renderer, params = {}) {
    ...
    this.separateShader = new RawShaderMaterial({
      uniforms: {
        inputTexture: { value: this.colorPass.texture },
      },
      vertexShader: orthoVertexShader,
      fragmentShader: separateFragmentShader,
      glslVersion: GLSL3,
    });
    this.separatePass = new ShaderPass(this.separateShader, {
      format: RGBAFormat,
      type: UnsignedByteType,
      minFilter: LinearFilter,
      magFilter: LinearFilter,
  ...
  render(scene, camera) {
    ...
    this.separatePass.render(this.renderer);

    this.bloomPass.source = this.separatePass.texture;
